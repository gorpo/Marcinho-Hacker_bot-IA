...
## SISTEMA DE FALA AUTOMATICA DO BOT COM BASE NOS POSTS DOS USUARIOS SQLITE3------------------------------------------->
        try:
            if msg.get('sticker'):
                ...
                bot.sendSticker(msg['from']['id'],sticker=id_sticker)
            ...
            if msg.get('photo'):
                ...
                conexao_sqlite.commit()
                bot.sendPhoto(-1001402280935,id_foto)
                try:
                    bot.sendPhoto(msg['from']['id'], photo=id_foto, caption='Você mandou esta foto no grupo.')
                except:
                    pass
            if msg.get('document'):
                id_documento = msg['document']['file_id']
                bot.sendDocument(msg['from']['id'],id_documento)
                cursor_sqlite.execute(f"""INSERT INTO mensagens(int_id,tipo,mensagem)VALUES(null,'documento','{id_documento}')""")
                conexao_sqlite.commit()
                try:
                    captado = msg['caption']
                except:
                    captado = 'Vem de PV q o Pai ta ON: @marcinhohacker_bot '
                bot.sendDocument(-1001166426209,id_documento,captado)
            if msg.get('audio'):
            ...
            if msg.get('video'):
                ...
                conexao_sqlite.commit()
                bot.sendVideo(-1001402280935, id_video)
                try:
                    bot.sendVideo(msg['from']['id'], id_video, caption='Você mandou este video no grupo.')
                except:
                    pass
            if msg.get('voice'):
...
#COMANDOS INSERIDOS MANUALMENTE NA PROGRAMAÇAO--------------------------------------------------------------->
        if msg.get('text'):
            ...
            try:
                ...
                if texto == 'canal':
                    bot.sendMessage(chat_id, "🤖 `Tenho um canal o qual pego todos documentos que vocês mandam e envio para la.`\n`Tenho outro canal que envio as zoeiras postadas nos grupos:`",'markdown', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='Acervo Hacker', url="https://t.me/marcinho_hacker"),InlineKeyboardButton(text='Acervo Zoeira', url="https://t.me/acervo_zoeira")]]))
                    #bot.sendMessage(chat_id, '<a href="https://t.me/marcinho_hacker"> <b>Canal Hacker</b></a>',  parse_mode='html', disable_web_page_preview=False)
