    ...
    try:
        ...
        if msg.get('text'):#ALTERAR A FREQUENCIA DO BOT------------------------------------------------------------------------------>
            ...
            if contador < frequencia:  # se o contador for menor que a frequencia o bot entrara na conversa enviando alguma mensagem salva no banco de dados
                ...
                mensagem_database_sqlite = mensagens_sqlite[randomico][2]
                ...
                if mensagem_database_sqlite.startswith('frequencia') or mensagem_database_sqlite.startswith('#') or mensagem_database_sqlite.startswith('marcinho'):
                    ...
                    if tipo_mensagem == 'imagem':
                        bot.sendPhoto(chat_id, photo= mensagem_database_sqlite)
                        print(f'🤖 Bot enviou uma imagem no grupo {msg["chat"]["title"]} com a id {mensagem_database_sqlite}')
                    if tipo_mensagem == 'documento':
                        bot.sendDocument(chat_id, document=mensagem_database_sqlite)
                        print(f'🤖 Bot enviou um documento no grupo {msg["chat"]["title"]} com a id {mensagem_database_sqlite}')
                    if tipo_mensagem == 'audio':
                        bot.sendAudio(chat_id, audio=mensagem_database_sqlite)
                        print(f'🤖 Bot enviou um audio no grupo {msg["chat"]["title"]} com a id {mensagem_database_sqlite}')
                    if tipo_mensagem == 'video':
                        bot.sendVideo(chat_id, video= mensagem_database_sqlite)
                        print(f'🤖 Bot enviou um video no grupo {msg["chat"]["title"]} com a id {mensagem_database_sqlite}')
                    if tipo_mensagem == 'texto':
                        bot.sendMessage(chat_id, mensagem_database_sqlite)
                        print(f'🤖 Bot enviou no grupo {msg["chat"]["title"]}: {mensagem_database_sqlite}')
                    if tipo_mensagem == 'sticker':
                        bot.sendSticker(chat_id, sticker=mensagem_database_sqlite)
                        print(f'🤖 Bot enviou sticker no grupo {msg["chat"]["title"]} com a id {mensagem_database_sqlite}')
